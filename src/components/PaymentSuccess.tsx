...
export default function PaymentSuccess({ registration }: PaymentSuccessProps) {
  ...
  const transactionId = registration.payment_transaction_id || 'Verified by event payment administrator';
  return (
    <div className="bg-white rounded-2xl border border-summit-orange-100 shadow-xl shadow-summit-orange-900/5 overflow-hidden animate-scale-in">
      <div className="bg-gradient-to-br from-summit-orange-600 to-summit-orange-700 px-6 py-8 text-center">
        ...
        <div className="text-xs font-semibold text-white/70 uppercase tracking-widest mb-1">Payment Verified</div>
        <h3 className="font-display font-bold text-2xl text-white mb-1">🎉 Congratulations!</h3>
        <p className="text-sm text-white/75">Your registration process is complete.</p>
      </div>
      <div className="p-6 lg:p-8">
        ...
        <div className="mt-5 bg-summit-orange-50 rounded-xl px-4 py-4">
          <p className="text-sm text-summit-charcoal/80 font-semibold">PAYMENT SUCCESSFUL</p>
          <p className="text-sm text-summit-charcoal/70 mt-1">Payment Amount: ₹10</p>
          <p className="text-sm text-summit-charcoal/70 mt-1 break-all">Transaction ID: {transactionId}</p>
        </div>
...
function generateConfirmationText(reg: RegistrationRecord): string {
  ...
  const transactionId = reg.payment_transaction_id || 'Verified by event payment administrator';
